...
export default function OwnerDashboard() {
    ...
    fetch(`${API}/api/kyc/status`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(data => {
        if (data.success) {
          ...
          setKycRejectionReason(data.data.kycRejectionReason || '');
        }
      })
      .catch(() => {});
  }, []);
  ...
  const handleKycSubmit = async () => {
    ...
    if (!nicFront || !nicBack || !selfie) {
      setKycMessage('Please select all three documents.');
      return;
    }
    ...
    try {
      ...
      setKycMessage(err.message || 'Upload failed. Try again.');
    } finally {
  ...
  // KYC Banner — shown on both desktop and mobile
  const KycBanner = () => {
    if (kycStatus === 'approved') return null;
    const configs: Record<string, { bg: string; border: string; icon: string; title: string; body: string; btn?: string }> = {
      not_submitted: {
        bg: 'bg-amber-500/10', border: 'border-amber-500/30', icon: '⚠️',
        title: 'KYC Verification Required',
        body: 'Submit your NIC and selfie to unlock all owner features and list your properties.',
        btn: 'Submit Documents',
      },
      pending: {
        bg: 'bg-blue-500/10', border: 'border-blue-500/30', icon: '🕐',
        title: 'KYC Under Review',
        body: 'Your documents have been submitted and are being reviewed by our team. This usually takes 1–2 business days.',
      },
      rejected: {
        bg: 'bg-red-500/10', border: 'border-red-500/30', icon: '❌',
        title: 'KYC Rejected',
        body: kycRejectionReason ? `Reason: ${kycRejectionReason}` : 'Your documents were rejected. Please resubmit.',
    ...
    if (!c) return null;
    return (
      <div className={`mx-4 mt-4 lg:mx-6 lg:mt-6 p-4 rounded-xl border ${c.bg} ${c.border} flex items-start gap-3`}>
        ...
        {c.btn && (
          <button
            ...
            className="flex-shrink-0 text-xs font-semibold px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white transition-colors border border-white/20"
          >
            {c.btn}
          </button>
        )}
  ...
  const KycModal = () => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div className="w-full max-w-md bg-[#1e2436] rounded-2xl border border-white/10 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-white">Submit KYC Documents</h2>
          <button onClick={() => { setShowKycModal(false); setKycMessage(''); }} className="text-gray-400 hover:text-white transition-colors">
            <X size={20} />
          </button>
        </div>

        <p className="text-xs text-gray-400 mb-5">Upload clear photos of your NIC (front & back) and a selfie holding the NIC. Max 5 MB each. JPG, PNG, or PDF.</p>

        {(['nicFront', 'nicBack', 'selfie'] as const).map((field) => {
          const labels = { nicFront: 'NIC Front', nicBack: 'NIC Back', selfie: 'Selfie with NIC' };
          return (
            <div key={field} className="mb-3">
              <label className="block text-xs font-medium text-gray-300 mb-1">{labels[field]}</label>
              <label className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border cursor-pointer transition-colors ${kycFiles[field] ? 'border-green-500/50 bg-green-500/5 text-green-400' : 'border-white/10 bg-white/5 text-gray-400 hover:border-cyan-400/50'}`}>
                <Upload size={14} />
                <span className="text-xs truncate">{kycFiles[field] ? kycFiles[field]!.name : 'Choose file…'}</span>
        ...
        {kycMessage && (
          <p className={`text-xs mt-3 px-3 py-2 rounded-lg ${kycMessage.includes('failed') || kycMessage.includes('Please') ? 'bg-red-500/10 text-red-400 border border-red-500/30' : 'bg-green-500/10 text-green-400 border border-green-500/30'}`}>
            {kycMessage}
          </p>
        )}
        ...
        <button
          ...
          className="w-full mt-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
        >
          {kycUploading ? <><Upload size={14} className="animate-bounce" />Uploading…</> : 'Submit for Review'}
        </button>
      </div>
    </div>
  );
  ...
  const ReviewModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowReview(false)}>
        <div className="bg-[#1e2845] rounded-2xl border border-white/10 w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowReview(false)} className="p-1.5 hover:bg-white/10 rounded-lg text-slate-400"><X size={16} /></button>
          </div>
          {done ? (
            <div className="flex flex-col items-center py-6 gap-3 text-center">
              ...
              <button onClick={() => setShowReview(false)} className="mt-2 px-6 py-2 bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-sm font-semibold rounded-xl">Close</button>
            </div>
          ) : (
            <>
              <div>
                ...
                <div className="flex gap-1">
                  {[1,2,3,4,5].map(n => (
                    <button key={n} type="button" onClick={() => setRating(n)} className={`transition-colors ${n <= rating ? 'text-yellow-400' : 'text-slate-600 hover:text-yellow-300'}`}>
              ...
              <div>
                ...
                <textarea value={comment} onChange={e => setComment(e.target.value)} rows={4} maxLength={1000}
                  placeholder="Share your experience with BoardingBook…"
              ...
              {error && <div className="flex items-center gap-2 text-xs text-red-400 bg-red-500/10 border border-red-500/20 px-3 py-2 rounded-lg"><AlertCircle size={13} />{error}</div>}
              <button onClick={submit} disabled={loading}
                className="w-full py-2.5 bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-sm font-semibold rounded-xl hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2">
  ...
  const TicketModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowTicket(false)}>
        <div className="bg-[#1e2845] rounded-2xl border border-white/10 w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-white text-lg">Create Support Ticket</h3>
            <button onClick={() => setShowTicket(false)} className="p-1.5 hover:bg-white/10 rounded-lg text-slate-400"><X size={16} /></button>
          </div>
          {done ? (
            <div className="flex flex-col items-center py-6 gap-3 text-center">
              <CheckCircle className="text-green-400" size={40} />
              ...
              <button onClick={() => setShowTicket(false)} className="mt-2 px-6 py-2 bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-sm font-semibold rounded-xl">Close</button>
            </div>
          ) : (
            <>
              <div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowReview(true)}
                  className="flex items-center gap-2 px-4 py-2 bg-yellow-500/10 border border-yellow-500/20 hover:border-yellow-400/50 text-yellow-400 text-sm font-medium rounded-xl transition-all">
                  <Star size={15} /> Write a Review
                </button>
                <button onClick={() => setShowTicket(true)}
                  className="flex items-center gap-2 px-4 py-2 bg-cyan-500/10 border border-cyan-500/20 hover:border-cyan-400/50 text-cyan-400 text-sm font-medium rounded-xl transition-all">
                  <LifeBuoy size={15} /> Get Support
                </button>
              </div>
      ...
      <div className="px-3 py-3 max-w-7xl mx-auto">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setShowReview(true)}
                className="flex items-center justify-center gap-1.5 py-2.5 bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-xs font-medium rounded-xl">
                <Star size={13} /> Write Review
              </button>
              <button onClick={() => setShowTicket(true)}
                className="flex items-center justify-center gap-1.5 py-2.5 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-medium rounded-xl">
                <LifeBuoy size={13} /> Get Support
              </button>
            </div>
